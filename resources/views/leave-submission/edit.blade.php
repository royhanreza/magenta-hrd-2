              ...
              <div class="card-body">
                <div class="row justify-content-between">
                  <div class="col-md-9">
                    ...
                    <div class="form-group row justify-content-between">
                      <label class="col-sm-3 col-form-label">Jatah Cuti Tahunan</label>
                      <div class="col-sm-9">
                        <input type="text" v-model="leaveAllowance" class="form-control form-control-sm" readonly required>
                      </div>
                    </div>
                    <div class="form-group row justify-content-between">
                      <label class="col-sm-3 col-form-label">Jumlah Telah Diambil</label>
                      <div class="col-sm-9">
                    ...
                    <div class="form-group row justify-content-between">
                      <!-- <label for="first-name" class="col-sm-3 col-form-label">Tanggal Cuti Sehari Penuh<sup class="text-danger">*</sup></label> -->
                      <label for="first-name" class="col-sm-3 col-form-label">Tanggal Cuti<sup class="text-danger">*</sup></label>
                      <div class="col-sm-9">
                        <!-- <input v-model="firstName" type="text" class="form-control form-control-sm" required> -->
                        <div class="input-group">
                          <input type="text" id="full-day-leave" class="form-control form-control-sm input-date-bs effective-date" aria-describedby="basic-addon2" readonly required>
                    ...
                    <!-- <div class="form-group row justify-content-between">
                      <label for="first-name" class="col-sm-3 col-form-label">Tanggal Cuti Setangah Hari<sup class="text-danger">*</sup></label>
                      <div class="col-sm-9">
                        
                        <div class="input-group">
                          <input type="text" id="half-day-leave" class="form-control form-control-sm input-date-bs " aria-describedby="basic-addon2" readonly required>
                          <div class="input-group-append">
                            <span class="input-group-text" id="basic-addon2"><i class="fas fa-calendar" style="line-height: 0;"></i></span>
                          </div>
                        </div>
                    ...
                    <div class="form-group row justify-content-between">
                      <label class="col-sm-3 col-form-label">Jumlah Hari Pengambilan</label>
                      <div class="col-sm-9">
                        <input type="text" v-model="totalTakenLeave" class="form-control form-control-sm" readonly required>
                        <span v-if="totalTakenLeave > leaveAllowance"><i class="fas fa-exclamation-triangle fa-xs text-warning"></i> Jumlah hari telah melewati batas maksimal</span>
                      </div>
                    </div>
                    <!-- <div class="form-group row justify-content-between">
                      ...
                      <div class="col-sm-9">
                        <div class="input-group">
                          <input type="file" class="form-control form-control-sm" aria-describedby="basic-addon2">
                          <div class="input-group-append">
...
<script>
  ...
  let app = new Vue({
    ...
    data: {
      ...
      employee: '{{ $leave_submission->employee_id }}',
      leaveAllowance: Number('{{ $remaining_leaves["total_leave"] }}'),
      leaveTaken: Number('{{ $remaining_leaves["taken_leave"] }}'),
      leaveTakenCurrentMonth: Number('{{ $remaining_leaves["taken_leave_current_month"] }}'),
      fullDayLeaveDates: String('{{ $leave_submission->leave_dates }}').split(','),
      oldFullDayLeaveDates: String('{{ $leave_submission->leave_dates }}').split(','),
      halfDayLeaveDates: [],
      // category: '',
      // permissionDates: [],
      // numberOfDays: '',
      // attachment: '',
      description: '{{ $leave_submission->description }}',
      status: 'pending',
      approve: false,
      loading: false,
    },
    methods: {
      ...
      sendData: function() {
        ...
        let data = {
          ...
          full_day_leave_dates: vm.fullDayLeaveDatesString,
          old_full_day_leave_dates: vm.oldFullDayLeaveDatesString,
        ...
        axios.patch('/api/leave-submissions/{{ $leave_submission->id }}', data)
          .then(function(response) {
            ...
            Swal.fire(
              ...
              if (result.isConfirmed) {
                window.location.href = '/leave/submission';
              }
            })
    ...
    computed: {
      ...
      // },
      fullDayLeaveDatesString: function() {
        return this.fullDayLeaveDates.map(date => moment(date).format("YYYY-MM-DD")).join();
      },
      oldFullDayLeaveDatesString: function() {
        return this.oldFullDayLeaveDates.map(date => moment(date).format("YYYY-MM-DD")).join();
      },
...
<script>
  $(function() {
    ...
    $("#employee-name").select2({
      ...
      axios.get(`/api/employees/${data.id}/remaining-leaves`, data)
        .then(function(response) {
          app.$data.leaveAllowance = response.data.data.total_leave;
          app.$data.leaveTaken = response.data.data.taken_leave;
          app.$data.leaveTakenCurrentMonth = response.data.data.taken_leave_current_month;
          // console.log(response);
        })
        .catch(function(error) {
          console.log(error.response);
    ...
    $('#full-day-leave').datepicker({
      format: 'yyyy-mm-dd',
      multidate: true,
    }).on('changeDate', function(e) {
      // `e` here contains the extra attributes
      app.$data.fullDayLeaveDates = e.dates;
    });

    $('#full-day-leave').datepicker('setDates', String('{{ $leave_submission->leave_dates }}').split(','))

    $('#half-day-leave').datepicker({
      format: 'yyyy-mm-dd',
      multidate: true,
    }).on('changeDate', function(e) {
      // `e` here contains the extra attributes
      // app.$data.permissionDates = e.dates;
      app.$data.halfDayLeaveDates = e.dates;
    });
  })
</script>
@endsection