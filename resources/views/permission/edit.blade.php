              ...
              <div class="card-body">
                <div class="row justify-content-between">
                  <div class="col-md-9">
                    ...
                    <div class="form-group row justify-content-between">
                      <label for="first-name" class="col-sm-3 col-form-label">Ganti Surat Keterangan Izin</label>
                      <div class="col-sm-9">
                        <!-- <input v-model="firstName" type="text" class="form-control form-control-sm" required> -->
                        <div class="input-group">
                      ...
                      <div class="col-sm-9">
                        <div class="card">
                          <div class="card-body d-flex align-items-center">
                            ...
                            <div class="pl-3">
                              <strong class="d-block">{{ $permission->attachment }}</strong>
                              Link: <a href="{{ ($permission->attachment !== null) ? Storage::disk('s3')->url($permission->attachment) : '' }}" target="_blank"><span class="text-muted">{{ ($permission->attachment !== null) ? Storage::disk('s3')->url($permission->attachment) : 'No File' }}</span></a>
                            </div>
                          </div>
                        </div>
                      </div>
...
<script>
  ...
  let app = new Vue({
    ...
    data: {
      ...
      category: '{{ $permission->permission_category_id }}',
      categories: JSON.parse('{!! $categories !!}'),
      permissionDates: String('{{ $permission->permission_dates }}').split(','),
      oldPermissionDates: String('{{ $permission->permission_dates }}').split(','),
      numberOfDays: '',
      attachment: '',
    ...
    methods: {
      ...
      sendData: function() {
        ...
        let data = {
          ...
          old_permission_dates: vm.oldPermissionDatesString,
        ...
        axios.post('/api/permission-submissions/{{ $permission->id }}', formData)
...
<script>
  $(function() {
    ...
    $('#permission-dates').datepicker('setDates', String('{{ $permission->permission_dates }}').split(','))
  })
</script>
@endsection