                ...
                <form @submit.prevent="addCareer">
                  ...
                  <div class="mt-5">
                    <div class="d-flex justify-content-end">
                      <button type="submit" class="btn btn-primary btn-sm" v-bind:disabled="loading || selectedPayslips.length < 1"><span v-if="loading" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>&nbsp;Save</button>
                    </div>
                  </div>
                  <p v-if="selectedPayslips.length < 1" class="text-right mt-3"><i class="fas fa-exclamation-triangle text-warning fa-xs"></i> Pilih minimal 1 slip gaji di tab Remunerasi</p>
                </form>
              </div>
                    ...
                    <ul class="list-group">
                      <li v-for="(payslip, index) in payslips" :key="payslip.id" class="list-group-item d-flex justify-content-between align-items-center" :class="{'bg-light': setDisablePayslip(payslip.id)}">
                        ...
                        <label class="custom-control custom-checkbox">
                          <input type="checkbox" :value="payslip.id" v-model="checkedPayslips" @change="selectPayslip(payslip.id)" class="custom-control-input" :disabled="setDisablePayslip(payslip.id)"><span class="custom-control-label"></span>
                        </label>
                      </li>
                    </ul>
                  </div>
...
<script>
  let app = new Vue({
    ...
    data: {
      ...
      selectedPayslips: [],
      selectedPayslips2: JSON.parse('{!! $selected_payslips !!}'),
      selectedPayslipWithBasicSalary: '',
      checkedPayslips: JSON.parse('{!! $checked_payslips !!}'),
      // payslipComponentModels: [],
      nonRemuneration: {
        employeeStatus: '{{ $career->employee_status }}',
        type: '{{ $career->type }}',
        department: '{{ $career->department_id }}',
        designation: '{{ $career->designation_id }}',
        jobTitle: '{{ $career->job_title_id }}',
        golongan: '',
        effectiveDate: '{{ $career->effective_date }}',
        endOfEmployementDate: '{{ $career->end_of_employement_date }}',
        endOfEmployeeStatusReminder: '{{ $career->end_of_employee_status_reminder }}',
        minimumWage: '{{ $career->province_minimum_wage_id }}',
        taxCalculationMethod: '{{ $career->tax_calculation_method }}',

      },
      loading: false,
    },
    mounted: function() {
      const selectedPayslipIds = this.selectedPayslips2.map(payslip => payslip.id);
      if (selectedPayslipIds.length > 0) {
        for (let i = 0; i < selectedPayslipIds.length; i++) {
          this.selectPayslip(selectedPayslipIds[i]);
        }
      }
    },
    methods: {
...
<script>
  $(function() {
    ...
    //   startDate: '{{ ($career->end_of_employement_date !== null) ? $career->end_of_employement_date : "" }}',
    //   format: 'yyyy-mm-dd',
    // })

    // $('#end-employement-reminder').datepicker({
    //   startDate: '{{ ($career->end_of_employemee_status_reminder !== null) ? $career->end_of_employemee_status_reminder : "" }}',
    //   format: 'yyyy-mm-dd',
    // })

    $('.effective-date').datepicker({
        format: 'yyyy-mm-dd',
        startDate: '{{ ($career->effective_date !== null) ? $career->effective_date : "" }}',
      })
