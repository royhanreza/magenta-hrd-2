      ...
      <div class="row">
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
          <div class="page-header">
            <h2 class="pageheader-title">Penggajian (Cuti) </h2>
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
          <div class="card">
            <div class="card-header d-flex">
              <h5 class="card-header-title">Cuti Diuangkan</h5>
            </div>
            <div class="card-body">
              <form>
                <div class="form-row">
              ...
              <div class="row">
                  <div class="col-lg-4 col-md-12">
                      <div class="card">
                        <div class="card-body">
                            <p>Total Dibuat</p>
                            <h2 class="text-right">Rp @{{ Intl.NumberFormat('de-DE').format(totalFinalPayslips) }}</h2>
                        </div>
                      </div>
                  </div>
                  <div class="col-lg-4 col-md-12">
                      <div class="card">
                        <div class="card-body">
                            <p>Total Belum Dibuat</p>
                            <h2 class="text-right">Rp @{{ Intl.NumberFormat('de-DE').format(totalPayslips) }}</h2>
                        </div>
                      </div>
                  </div>
                  <div class="col-lg-4 col-md-12">
                      <div class="card">
                        <div class="card-body">
                            <p>Total</p>
                            <h2 class="text-right">Rp @{{ Intl.NumberFormat('de-DE').format(totalFinalPayslips + totalPayslips) }}</h2>
                        </div>
                      </div>
                  </div>
              </div>
              @if(in_array("addLeaveSalary", $userLoginPermissions))
              ...
              <div class="table-responsive">
                <table class="table table-bordered use-datatable">
                  ...
                  <tbody>
                    <tr is="final-payslip" v-for="finalPayslip in finalPayslips" :key="finalPayslip.id" :id="finalPayslip.id" :name="finalPayslip.employee.first_name" :leave="finalPayslip?.income[0]?.final_leave" :thp="finalPayslip?.income[0]?.value" :ondelete="deletePayslip"></tr>
                    <tr is="payslip" v-for="career in careers" :key="career.id" :id="career.id" :name="career.employee.first_name" :leave="career.final_leave" :thp="career.amount" :checkedcareers.sync="checkedCareers" :onshowmodal="showModal"></tr>
                  </tbody>
                </table>
              </div>
            </div>
          ...
          <div class="row">
            <div class="col-md-6">
              <table class="table">
                ...
                <tr>
                  <td>Cuti Diuangkan</td>
                  <td class="d-flex justify-content-end">
                    <div class="input-group" style="width: 200px;">
                      <div class="input-group-prepend">
                        <span class="input-group-text">Rp</span>
                      </div>
                      <input type="text" :value="Intl.NumberFormat('de-DE').format(careers[selectedCareerIndex]?.amount)" class="form-control text-right" aria-describedby="basic-addon1" readonly>
                    </div>
                  </td>
                </tr>
                ...
                <tr class="bg-light">
                  ...
                  <td class="d-flex justify-content-end">
                    <div class="input-group" style="width: 200px;">
                      ...
                      <input type="text" :value="calculateTakeHomePay(careers[selectedCareerIndex]?.amount, 0)" class="form-control text-right" aria-describedby="basic-addon1" readonly>
                    </div>
                  </td>
                </tr>
...
<script>
  ...
  Vue.component('payslip', {
    props: ['id', 'name', 'leave', 'thp', 'checkedcareers', 'onshowmodal'],
    computed: {
      temp: {
        get: function() {
  ...
  Vue.component('final-payslip', {
    props: ['id', 'name', 'leave', 'thp', 'ondelete'],
    template: `
    <tr>
      <td class="text-center" style="width: 60px;">
      </td>
      ...
      <td class="text-center">
        <div class="btn-group" role="group" aria-label="Basic example">
          ...
          @if(in_array("deleteLeaveSalary", $userLoginPermissions))
          <a href="#" @click.prevent="ondelete(id)" class="btn btn-light btn-sm"><i class="fas fa-trash-alt"></i></a>
          @endif
          @if(in_array("editLeaveSalary", $userLoginPermissions))
  ...
  let app = new Vue({
    ...
    computed: {
     ...
     totalFinalPayslips: function() {
         let vm = this;
         if(!Array.isArray(vm.finalPayslips)) {
             return 0;
         }
         const total = vm.finalPayslips.map(finalPayslip => {
             return Number(finalPayslip?.income[0]?.value);
         }).reduce((acc, cur) => {
             return acc + cur;
         }, 0);
     ...
     totalPayslips: function() {
         let vm = this;
         if(!Array.isArray(vm.careers)) {
             return 0;
         }
         const total = vm.careers.map(career => {
             return Number(career?.amount);
         }).reduce((acc, cur) => {
             return acc + cur;
         }, 0);
         return total;
