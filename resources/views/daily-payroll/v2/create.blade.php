                ...
                <tr>
                  ...
                  <td class="text-right"><strong>Rp @{{ calculateTakeHomePay(generatedPayslips[selectedGeneratedPayslipIndex]?.payments) }}</strong></td>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                <tr v-for="deduction in generatedPayslips[selectedGeneratedPayslipIndex]?.deductions">
                                    <td>@{{ deduction.name }}</td>
                                    <td class="text-right">@{{ Intl.NumberFormat('de-DE').format(deduction.value) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
<script>
    ...
    let app = new Vue({
        ...
        methods: {
            ...
            calculateDeductions: function(deductions) {
                if (Array.isArray(deductions) && deductions.length > 0) {
                    return deductions.map(deduction => Number(deduction.value)).reduce((acc, cur) => {
                        return acc + cur;
                    }, 0)
                }
            ...
            calculateTakeHomePay: function(payments, deductions) {
                ...
                return takeHomePay - this.calculateDeductions(deductions);
            },
